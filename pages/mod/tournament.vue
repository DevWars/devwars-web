    ...
    <div>
        <div class="modpanel__header">
            ...
            <div class="modpanel__header-actions">
                ...
                <button v-async-click="[endTournament]" class="btn btn-danger" v-if="tournament.active && !tournament.done">
                    End
                </button>
                <button
                    v-show="!tournament.done && !tournament.active" v-async-click="[activate]"
        ...
        <nav class="nav-tabs">
            <nuxt-link :to="'/mod/tournament/details?tournament=' + tournament.id" class="nav-tabs__item">Details</nuxt-link>
        </nav>
...
<script>
    ...
    export default class DashboardTournament extends Vue {
        ...
        async save() {
            ...
            const transformed = this.tournament.objectives.map(it => ({ ...it, tournament: null }));
