  ...
  <div>
    <PanelHeader :title="startDate" :subtitle="`@ ${startTime} UTC`">
      <ButtonGroup>
        ...
        <Button v-if="viewingSetupPage" class="primary" :async-click="save">
          Save
        </Button>
        <Button
        ...
        <Button v-if="schedule.status === 1" class="danger" :async-click="end">
          End
        </Button>
        <Button v-if="user.role === 'ADMIN'" class="outline danger" disabled>
          Delete
        </Button>
      </ButtonGroup>
    </PanelHeader>
    ...
    <Tabs>
      ...
      <nuxt-link :to="`/mod/schedule/applications?schedule=${schedule.id}`">
        Applications
      </nuxt-link>
    </Tabs>
